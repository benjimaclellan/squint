...
class FockState(AbstractState):
    n: Sequence[tuple[complex, Sequence[int]]] # todo: add superposition as n, using second typehint
...
class BeamSplitter(AbstractGate):
    ...
    @beartype
    def __init__(self, wires: tuple[int, int], r: ArrayLike = jnp.array(jnp.pi / 4), phi=jnp.array(0.0)):
        super().__init__(wires=wires)
        self.r = jnp.array(r)
        self.phi = jnp.array(phi)
        return
