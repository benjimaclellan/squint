...
class Wire(eqx.Module):
    ...
    def __init__(
        ...
        """
        if dim < 1:
            raise ValueError("Dimension should be 1 or greater.")
        if isinstance(idx, int):
            if idx < 0:
                raise ValueError(
                    "Wire.idx integers must be positive. Negative integers are reserved for autogenerated identities."
                )
        self.dim = dim
        self.dof = dof
        # self.info = info
...
class AbstractContainer(eqx.Module):
    """ """

    pass  # TODO: flesh this out
...
class Block(AbstractContainer):
    ...
    ops: OrderedDict[Union[str, int], Union[AbstractProcess, AbstractContainer]]
