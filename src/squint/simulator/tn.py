...
def _compile(
    ...
    **kwargs,
    ...
    circuit_tn = TNCircuit(circuit)
    
    path, info = circuit_tn.path(optimize=optimize)
    backend = circuit_tn.backend
    wires = circuit_tn.wires
    wires_ptrace = set(
        sum(
            (
                op.wires
                for op in circuit_tn.unwrap()
    ...
    _tensor = functools.partial(
        _tensor_func,
        subscripts=circuit_tn.subscripts,
        path=path,
        backend=backend,
    )
