...
_subscripts = [
    ...
    f"{
        ' '.join(
            [
                get_symbol(2 * k) if k in (i, j) else '1'
                for k in range(len(wires))
            ] + 
            [
                get_symbol(2 * k + 1) if k in (i, j) else '1'
                for k in range(len(wires))
            ]
        )
    }"
...
_h = sum(
    [r * einops.rearrange(_r, subscript) for r, subscript in zip(rs, _subscripts)] +
    [r.conj() * einops.rearrange(_l, subscript) for r, subscript in zip(rs, _subscripts)]
...
d = {i: 'yes', 2: 'no'}
_h = sum([
    functools.reduce(jnp.kron, [{i: r * create(dim), j: destroy(dim)}.get(k, eye(dim)) for k in range(len(wires))])
    for r, (i, j) in zip(rs, combs)
] + 
    [
    functools.reduce(jnp.kron, [{j: r.conj() * create(dim), i: destroy(dim)}.get(k, eye(dim)) for k in range(len(wires))])
    for r, (i, j) in zip(rs, combs)
]
    )
...
i, j = 1, 2
[
    [{j: jnp.conjugate(r) * create(dim), i: destroy(dim)}.get(k, eye(dim)) for k in range(len(wires))]
    for r, (i, j) in zip(rs, combs)
]
