...
from squint.interface.base import Block, Circuit, SharedGate, Wire, ClassicalWire
...
from squint.interface.measurements import AbstractProjectiveMeasurement, ComputationalBasisMeasurement
...
if name == "qubit":
    ...
    circuit.add(RZGate(wires=(wire,), phi=0.2 * jnp.pi), "phase")
    circuit.add(HGate(wires=(wire,)))
    circuit.add(ComputationalBasisMeasurement(wires=(wire,), out=out),  "measure")
...
#%%
wires = [Wire(dim=2, idx=i) for i in range(1)]
pvm = ComputationalBasisMeasurement(wires=wires, out=out)
pvm()

#%%
circuit.ops['measure']()
#%%
cs, rhs = PostSquintWalk(MapTensorIndicesMixed())(circuit)
lhs = PostSquintWalk(CollectSubscripts())(cs)

print(lhs)
print(rhs)
print(cs.ops["measure"].subscripts)
...
subscripts = circuit_to_subscripts(_circuit, backend=backend)
print(subscripts)
...
#%%
subscripts, path = circuit_to_optimized_tensor_network_contraction_path(circuit, backend=backend)

def simulate(params):
    circuit_ = eqx.combine(params, static)  # static in closure
    tensors = circuit_to_tensors(circuit_, backend=backend)
